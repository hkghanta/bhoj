  ...
  return (
    <div className="flex">
      {Array.from({ length: 5 }, (_, i) => (
        <Star key={i} className={`h-4 w-4 ${i < rating ? 'fill-yellow-400 text-yellow-400' : 'text-text-4'}`} />
      ))}
    </div>
  )
}
    ...
    <div className="bg-white rounded-xl border p-5">
      <div className="flex items-start justify-between mb-2">
        <div>
          ...
          <p className="font-medium text-text-1 mt-1">{review.customer.name}</p>
          {review.event_type && <p className="text-xs text-text-4">{review.event_type}</p>}
        </div>
        <span className="text-xs text-text-4">{format(new Date(review.created_at), 'd MMM yyyy')}</span>
      </div>

      {(review.food_quality_rating || review.service_rating || review.value_rating) && (
        <div className="flex gap-4 text-xs text-text-4 mb-3">
      ...
      {review.title && <p className="font-medium text-text-1 mb-1">{review.title}</p>}
      {review.body && <p className="text-sm text-text-3">{review.body}</p>}

      {reply && (
        <div className="mt-3 pl-4 border-l-2 border-brand">
          <p className="text-xs text-brand font-medium mb-1">Vendor response</p>
          <p className="text-sm text-text-3">{reply}</p>
        </div>
      )}
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="flex gap-2">
            <button
              onClick={submitReply}
              disabled={submitting}
              className={cn(buttonVariants({ size: 'sm' }), 'bg-brand hover:bg-brand-hover disabled:opacity-50')}
            >
