...
export function PricingCard({ plan, tier, currentTier, onUpgrade, upgrading }: Props) {
  ...
  return (
    <div className={`rounded-xl border p-6 flex flex-col ${
      tier === 'PREMIUM' ? 'border-brand ring-2 ring-brand' :
      isCurrent ? 'border-green-300 bg-green-50' : ''
    }`}>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-text-1">{plan.name}</h3>
        {tier === 'PREMIUM' && <Badge className="bg-brand text-white">Most Popular</Badge>}
        {isCurrent && <Badge className="bg-green-600 text-white">Current Plan</Badge>}
      </div>

      <div className="mb-6">
        {plan.price === 0 ? (
          <span className="text-3xl font-bold text-text-1">Free</span>
        ) : (
          <div>
            <span className="text-3xl font-bold text-text-1">£{plan.price}</span>
            <span className="text-text-4">/month</span>
          </div>
        )}
      ...
      <ul className="space-y-2 flex-1 mb-6">
        {plan.features.map(feature => (
          <li key={feature} className="flex items-start gap-2 text-sm text-text-3">
      ...
      {isUpgrade && (
        <button
          ...
          className={cn(
            ...
            tier === 'PREMIUM' ? 'bg-brand hover:bg-brand-hover' : ''
          )}
        >
      ...
      {tier === 'FREE' && !isCurrent && (
        <p className="text-center text-sm text-text-4">Downgrade via billing portal</p>
      )}
    </div>
  )
}
