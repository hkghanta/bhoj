  ...
  return (
    <div className="border-r h-full w-72 overflow-y-auto bg-white flex-shrink-0">
      <div className="px-4 py-4 border-b">
        <h2 className="font-semibold text-text-1">Messages</h2>
      </div>
      {conversations.length === 0 && (
        <div className="p-6 text-center text-text-4 text-sm">No conversations yet.</div>
      )}
      {conversations.map(c => {
        ...
        return (
          <Link key={c.id} href={`${basePath}/${c.id}`}>
            <div className={cn(
              'flex items-start gap-3 px-4 py-3 border-b hover:bg-cream cursor-pointer',
              isActive && 'bg-cream border-l-2 border-l-brand'
            )}>
              <div className="w-10 h-10 rounded-full bg-cream flex items-center justify-center flex-shrink-0 text-brand font-semibold text-sm overflow-hidden">
                {avatar ? (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <span className="font-medium text-sm text-text-1 truncate">{name}</span>
                  {c.last_message_at && (
                    <span className="text-xs text-text-4 flex-shrink-0 ml-2">
                ...
                {lastMsg && (
                  <p className="text-xs text-text-4 truncate mt-0.5">{lastMsg.body}</p>
                )}
              </div>
              {c.unread_count > 0 && (
                <span className="bg-brand text-white text-xs rounded-full h-5 w-5 flex items-center justify-center flex-shrink-0">
                  {c.unread_count}
                </span>
              )}
            </div>
