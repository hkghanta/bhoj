...
export function PlaybookSuggestion({ eventId, eventType }: PlaybookSuggestionProps) {
  ...
  return (
    <section
      ...
      className="bg-white dark:bg-cream-2 rounded-2xl border border-brand-border overflow-hidden shadow-md"
    >
      <div className="bg-brand/5 border-b border-brand-border px-6 py-5 flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-lg font-extrabold tracking-tight text-text-1 mb-1 flex items-center gap-2">
      ...
      <div className="px-6 py-4 flex items-center gap-3 flex-wrap">
        <button
          ...
          disabled={loading}
          className="inline-flex items-center gap-2 rounded-xl bg-brand text-white px-5 py-2.5 text-sm font-semibold hover:bg-brand-hover transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {loading ? (
            <>
